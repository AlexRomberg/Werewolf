...
export class Cupid extends Character {
    ...
    override GetActions = () => [
        ...
        !this.isDone && $localize`:@@character-action-cupid-2:Muss ein Paar bestimmen`,
        $localize`:@@character-action-cupid-3:Das Paar muss erwachen`];
    override GetButtons = () => {
        const buttons = [];
        if (!this.isAssigned) {
        ...
        if (!this.isDone) {
            buttons.push({
                Title: $localize`:@@character-button-cupid-1:Paar zuweisen`, Action: this.requestCouple.bind(this)
            });
        }
        return buttons;
    };
    ...
    private async requestCouple({ GameState, Dialog }: { GameState: StateService, Dialog: DialogService }) {
        try {
            const people = await Dialog.ShowPeopleSelectionDialog($localize`:@@character-dialog-cupid-1:Ein Paar auswählen`, 2);
